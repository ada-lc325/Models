...
class DWTFunction_2D(Function):
    ...
    def forward(ctx, input, matrix_Low_0, matrix_Low_1, matrix_High_0, matrix_High_1):
        ...
        # Matrices are float32; cast input to match (handles AMP float16 input)
        x = input.float()
    ...
    def backward(ctx, grad_LL, grad_LH, grad_HL, grad_HH):
        ...
        # Gradients may be float16 under AMP; cast to float32 to match saved matrices
        grad_LL, grad_LH = grad_LL.float(), grad_LH.float()
        grad_HL, grad_HH = grad_HL.float(), grad_HH.float()
...
class IDWTFunction_2D(Function):
    ...
    def forward(ctx, input_LL, input_LH, input_HL, input_HH,
        ...
        ctx.save_for_backward(matrix_Low_0, matrix_Low_1,
                              matrix_High_0, matrix_High_1)
        LL, LH = input_LL.float(), input_LH.float()
        HL, HH = input_HL.float(), input_HH.float()
        L = torch.add(torch.matmul(LL, matrix_Low_1.t()),
                      torch.matmul(LH, matrix_High_1.t()))
        H = torch.add(torch.matmul(HL, matrix_Low_1.t()),
